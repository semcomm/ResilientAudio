...
class SEANetEncoder(nn.Module):
    ...
    def __init__(self, channels: int = 1, dimension: int = 128, n_filters: int = 32, n_residual_layers: int = 1,
        ...
        print('Mult', mult, mult * n_filters * 2)
        ...
        for i, ratio in enumerate(self.ratios):
            ...
            self.strides.append(ratio)

    def remove_WN(self):
        for m in self.model.modules():
            if isinstance(m, SConv1d) :
                remove_weight_norm(m.conv.conv)
        print('Weight Norm removed')

    def forward(self, x):
        return self.model(x)

    def forward_before_lstm(self, x):
        return self.model[:-3](x)

    @staticmethod
    def _conv_output_size(input_size, stride):
        """Returns the length of the output size for a convolution.
        due to definition in ``SConv1d''
        ...
        where:
            ...
            S: `stride`
        """
        return (float(input_size - stride) / stride) + 1


    def output_len(self, input_len):
        """Returns the length of the CTC matrix for a given input seq. len."""

        output_len = input_len
        for stride in self.strides:
            output_len = SEANetEncoder._conv_output_size(output_len, stride)
