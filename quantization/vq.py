...
class ResidualVectorQuantizer(nn.Module):
    ...
    def encode(self, x: torch.Tensor, frame_rate: int, bandwidth: tp.Optional[float] = None) -> torch.Tensor:
        ...
        """
        n_q = self.get_num_quantizers_for_bandwidth(frame_rate, bandwidth)
        # print(self.n_q, n_q, frame_rate, bandwidth)
        # assert n_q <= self.n_q 
    ...
    def decode_with_different_stages(self, codes: torch.Tensor, valid_q_bkt_mask):
        return self.vq.decode_with_different_stages(codes, valid_q_bkt_mask)


if __name__ == '__main__':
    import torch

    rvq_quantizer = ResidualVectorQuantizer(
        dimension=128,
        bins=1024,
        n_q=4,
    )
    bw_per_q = rvq_quantizer.get_bandwidth_per_quantizer(16000 // 320)  # 0.5 kbps per q
    print(bw_per_q)
